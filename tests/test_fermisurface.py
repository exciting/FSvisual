...
def test_scale_surface(calc_fermisurface):
    ...
    with pytest.raises(ValueError, match="surface is not yet defined"):
        calc_fermisurface.scale_surface(5)
...
def test_slcie_surface_exception(calc_fermisurface):
...
def test_downsample_surface(calc_fermisurface, face_percentage, face_number, expected_mesh):

    with pytest.raises(ValueError, match="surface is not yet defined"):
        calc_fermisurface.downsample_surface(70, None)
    column = calc_fermisurface.energy_values.columns[3]
    calc_fermisurface.marching_cubes(column)

    calc_fermisurface.downsample_surface(face_percentage, face_number)

    assert np.allclose(expected_mesh.vertices, calc_fermisurface.surface.vertices)
    assert np.array_equal(expected_mesh.faces, calc_fermisurface.surface.faces)

def test_downsample_surface_exception(calc_fermisurface):

    column = calc_fermisurface.energy_values.columns[3]
    calc_fermisurface.marching_cubes(column)
    with pytest.raises(ValueError, match="You can only either provide a face_percentage or face_numbers, not both"):
        calc_fermisurface.downsample_surface(70, 1000)
